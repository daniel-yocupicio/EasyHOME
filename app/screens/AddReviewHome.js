...
export default function AddReviewHome(props) {
  const { navigation, route } = props;
  const { idHome } = route.params;
  ...
  const updateHome = () => {
    const homeRef = db.collection("homes").doc(idHome);

    homeRef.get().then((response) => {
      const homeData = response.data();
      const ratingTotal = homeData.ratingTotal + rating;
      const quantityVoting = homeData.quantityVoting + 1;
